...
class Command(BaseCommand):
    help = 'Update origin field for all models from one user to another.'
    ...
    def handle(self, *args, **options):
        ...
        for model in all_models:
            if 'origin' in [field.name for field in model._meta.fields]:
                queryset = model.objects.filter(origin=old_user)
                count = queryset.count()

                if count > 0:
                    self.stdout.write(
                        f"Found {count} instances in {model.__name__} with origin={old_user}."
                    )

                    if not dry_run:
                        queryset.update(origin=new_user)
                        self.stdout.write(
