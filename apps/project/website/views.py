...
from apps.project.website.forms import ProjectWebsiteForm
from apps.project.website.models import ProjectWebsite


class CreateWebsiteView(LoginRequiredMixin, ProjectContextMixin, FormView):
    ...
    def get_form_kwargs(self):
        ...
        existing = ProjectWebsite.objects.filter(project=self.get_project()).first()
    ...
    def form_valid(self, form):
        ...
        obj.save()
        return redirect('project:website:create', pk=self.get_project_id())

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        website = ProjectWebsite.objects.filter(project=self.get_project()).first()
        ctx['website'] = website
...
class PreviewView(LoginRequiredMixin, ProjectContextMixin, TemplateView):
    template_name = 'project/website/preview.html'

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        objects_filter = ProjectWebsite.objects.filter(pk=kwargs['website_pk'])
