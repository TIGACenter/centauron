...
class Command(BaseCommand):
    help = "Imports the scanner terminology."
    ...
    def handle(self, *args, **options):
        ...
        centauron_stainings_csv = finders.find('codesystems/scanners.csv')
        if centauron_stainings_csv is None:
            self.stderr.write('Scanners not found.')
            return

        codesystem_uri = 'http://centauron.net/scanners'
        codesystem_name = 'CENTAURON Scanners'
